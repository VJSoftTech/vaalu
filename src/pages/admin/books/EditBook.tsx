...
export default function EditBook() {
    ...
    Promise.all([
      ...
      .then(([book, authorsRes, catsRes]) => {
        reset({
          ...
          publisher_serial_number: book.publisher_serial_number ?? '',
        })
        setCoAuthorIds((book.authors ?? []).filter((a) => a.id !== book.author_id).map((a) => a.id))
        setEditorNames((book.editors ?? []).length > 0 ? (book.editors ?? []).map((e) => e.name) : [''])
  ...
  const checkDuplicateTitle = async (title: string) => {
    ...
    try {
      ...
      const dup = res.data.some((b) => b.id !== Number(id) && b.title.trim().toLowerCase() === trimmed.toLowerCase())
  ...
  const submitForm = async (data: BookFormData, force: boolean) => {
    ...
    try {
      ...
      toast({ title: message || 'Failed to update book', variant: 'destructive' })
    }
  }
      ...
      <Card>
        <CardContent className="p-6">
          <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-4 gap-y-3 items-start">
            ...
            {textFields.map(({ id: fid, label, type, required, placeholder }) => (
